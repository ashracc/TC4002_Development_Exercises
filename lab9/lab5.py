...
class Sort:
    ...
    def set_input_data(self, file_path):
        ...
        if not file.is_file():
            logger.error("Could not find {}".format(file_path))
        ...
        logger.info("Returning read data")
    ...
    def set_output_data(self, file_path):
        ...
        try:
            ...
            raise PermissionError("Could not write to " + file_path)
    ...
    def execute_bubble_sort(self, the_list):
        ...
        @par Example:
            ...
            [3, 5, 7, 8, 10]
        """
        logger.info("Sorting: {}".format(the_list))
        flag = True
        while flag:
            flag = False
            for idx in range(len(the_list) - 1):
                if the_list[idx] > the_list[idx + 1]:
                    ...
                    flag = True
        logger.info("Sorted: {}".format(the_list))
...
logging.basicConfig(level = logging.DEBUG,
                    format = LOG_FORMAT,
                    datefmt = "%Y/%m/%d %H:%M:%S",
                    handlers=[logging.FileHandler('lab5.log', mode='w'), logging.StreamHandler()]
                    )
